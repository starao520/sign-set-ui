...
export default {
  ...
  data() {
    return {
      loading: false,
      emailForm: { id: 1, fromUser: '', user: '', pass: '', host: '', port: '', sslEnable: '' },
      rules: {
        fromUser: [
  ...
  methods: {
    ...
    doSubmit() {
      this.$refs['emailForm'].validate((valid) => {
        if (valid) {
          this.loading = true
          update(this.emailForm).then(res => {
            this.$notify({
              title: '修改成功',
              type: 'success',
              duration: 2500
